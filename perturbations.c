...
int perturb_einstein(
  ...
  double k2,a,a2,a_prime_over_a,a_prime_over_a_prime;
  double s2_squared;
  double shear_g = 0.;  
  ...
  a2 = a * a;
  a_prime_over_a = ppw->pvecback[pba->index_bg_H]*a;
  a_prime_over_a_prime =ppw->pvecback[pba->index_bg_H_prime] *ppw->pvecback[pba->index_bg_a] + pow(ppw->pvecback[pba->index_bg_H] * ppw->pvecback[pba->index_bg_a],2); 
  s2_squared = 1.-3.*pba->K/k2;
  T = 6*pow(a_prime_over_a,2);
  b = 0.01;
  m = pow(27581.04,1-b)*(0.6919077)/(2*b-1);
  fTD = 1.-b*m*pow(-T,b-1.);
  fTD_prime=12.*m*b*(b-1.)*pow(-T,b-2.)*a_prime_over_a*a_prime_over_a_prime;
  beta_T = -(fTD_prime)/(3.*a_prime_over_a*fTD);
